...
1. If the component needs to make benign changes to its children in response to updates, this is best done in the `updateHandler()` mixin. Experience has taught that this is a cleaner approach because, as already mentioned, it does away with the need for a switch on the presence or otherwise of the `render()` method's 'update` argument.
